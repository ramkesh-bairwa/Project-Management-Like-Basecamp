...
export default function SiteSettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/settings', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json()).then(d => setForm(d));
  }, []);
            ...
            <div>
              ...
              <div style={{ background: primary, borderRadius: 10, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                {form.site_logo_url ? (
                  ...
                  <div style={{ borderRadius: 8, background: `linear-gradient(135deg, ${accent}, ${primary})`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 900, minWidth: 32, height: 32, padding: '0 7px', fontSize: (form.logo_letter?.length || 1) > 2 ? 10 : (form.logo_letter?.length || 1) > 1 ? 12 : 15 }}>
                    {form.logo_letter || 'P'}
                  </div>
                )}
