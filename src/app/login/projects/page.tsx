...
export default function ProjectsLoginPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: 'linear-gradient(135deg, #e63946 0%, #c1121f 100%)' }}>
      {/* Left Side - Branding */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '60px', color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '500px' }}>
          <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '12px', marginBottom: '40px', textDecoration: 'none' }}>
            ...
            <span style={{ fontSize: '28px', fontWeight: 900, color: '#fff' }}>ProjectHub</span>
          </Link>
          ...
          <p style={{ fontSize: '18px', opacity: 0.9, lineHeight: 1.7, marginBottom: '40px' }}>
            Complete project management solution with tasks, teams, timelines, and powerful collaboration tools. Everything you need to deliver projects on time.
          </p>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {[
              { icon: '✅', text: 'Task management with priorities & deadlines' },
              { icon: '📊', text: 'Visual dashboards and progress tracking' },
              { icon: '👥', text: 'Team collaboration and assignments' },
      ...
      <div style={{ width: '550px', background: '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px', boxShadow: '-10px 0 40px rgba(0,0,0,0.1)' }}>
        <div style={{ maxWidth: '400px', margin: '0 auto', width: '100%' }}>
          <div style={{ marginBottom: '40px' }}>
            <h2 style={{ fontSize: '32px', fontWeight: 900, color: '#1e293b', marginBottom: '8px' }}>Welcome back!</h2>
            <p style={{ color: '#64748b', fontSize: '15px' }}>Sign in to continue to Projects</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ marginBottom: '30px' }}>
            ...
            <div style={{ marginBottom: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <Link href="/forgot-password/projects" style={{ fontSize: '13px', color: '#e63946', textDecoration: 'none', fontWeight: 600 }}>
                  Forgot Password?
                </Link>
              </div>
              <input
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ width: '100%', padding: '14px', borderRadius: '10px', background: 'linear-gradient(135deg, #e63946 0%, #c1121f 100%)', color: '#fff', fontSize: '16px', fontWeight: 700, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, transition: 'all 0.2s', boxShadow: '0 4px 14px rgba(230, 57, 70, 0.4)' }}
              onMouseEnter={(e) => !loading && (e.currentTarget.style.transform = 'translateY(-2px)')}
              onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}
            >
              {loading ? 'Signing in...' : 'Sign In to Projects'}
            </button>
          </form>
          ...
          <div style={{ textAlign: 'center' }}>
            <span style={{ color: '#64748b', fontSize: '14px' }}>
              ...
              <Link href="/register/projects" style={{ color: '#e63946', fontWeight: 700, textDecoration: 'none' }}>
                Sign up
              </Link>
            </span>
          </div>
          ...
          <div style={{ marginTop: '24px', paddingTop: '24px', borderTop: '1px solid #e2e8f0', textAlign: 'center' }}>
            <Link href="/login/chatme" style={{ color: '#64748b', fontSize: '13px', textDecoration: 'none', fontWeight: 600 }}>
              Looking for ChatMe? <span style={{ color: '#667eea' }}>Sign in here →</span>
            </Link>
          </div>
        </div>
      </div>
