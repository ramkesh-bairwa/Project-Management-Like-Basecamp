...
export const POST = withAuth(async (req: NextRequest, user) => {
  ...
  if (!planId || expired) {
    return apiError('You must activate a plan before creating projects.', 403);
  }
  ...
  if (maxProjects !== -1) {
    ...
    if ((cnt?.c ?? 0) >= maxProjects) {
      return apiError(`Plan limit reached: your plan allows ${maxProjects} project${maxProjects === 1 ? '' : 's'}. Upgrade to create more.`, 403);
    }
  }
